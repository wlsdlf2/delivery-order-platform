...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse<OrderResponseDto>> getOrder(@PathVariable UUID orderId,
        ...
        OrderResponseDto dto = orderService.getOrder(orderId, impl.getUsername(),
    ...
    public ResponseEntity<ApiResponse<OrderResponseDto>> deleteOrder(@PathVariable UUID orderId,
        @AuthenticationPrincipal UserDetailsImpl impl) {
        OrderResponseDto dto = orderService.deleteOrder(orderId, impl.getUsername(),
            impl.getUser().getRole());
        return ResponseEntity.ok(ApiResponse.success(dto));
    }
     ...
     * @return
     */
    @PreAuthorize("hasRole('ROLE_MASTER') or hasRole('ROLE_OWNER')") // 관리자 혹은 가게 주인인 경우만 허용.
    @PatchMapping("/{orderId}/status")
    public ResponseEntity<ApiResponse<Void>> updateOrderStatus(
        @PathVariable UUID orderId, @RequestBody OrdeStatusRequestDto status,
        @AuthenticationPrincipal UserDetailsImpl impl) {
        //주문 상태 변경 메서드 호출.
        orderService.updateOrderStatus(orderId, status, impl.getUsername(),
            impl.getUser().getRole());
        return ResponseEntity.ok(ApiResponse.success());
    }
     ...
     * @param request 변경된 주문요청 데이터
     * @param impl    :          로그인한 사용자 정보
     * @return 주문 응답객체
     */
    @PreAuthorize("hasRole('ROLE_MASTER') or hasRole('ROLE_CUSTOMER')")// 관리자 혹은 고객만 접근 가능하도록 한다.ㄴ
    @PatchMapping("/{orderId}")
    public ResponseEntity<ApiResponse<Void>> updateOrderRequest(
        @PathVariable UUID orderId, @RequestBody String request,
        @AuthenticationPrincipal UserDetailsImpl impl) {
        // 주문사항 요청 메서드 호출.
        orderService.updateOrderRequest(orderId, request, impl.getUsername(),
            impl.getUser().getRole());
        return ResponseEntity.ok(ApiResponse.success());
    }
